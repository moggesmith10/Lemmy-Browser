...
export function Vote({post, updatePost} : {post: PostView, updatePost: Function}){
    ...
    return(
        <View style={VoteStyle.voteContainer}>
            <TouchableOpacity onPress={() => vote(post.my_vote != 1 ? 1 : 0)}>
                <Text style={[VoteStyle.vote, post.my_vote > 0 ? VoteStyle.voteUpEnabled:null]}>+</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => vote(post.my_vote != -1 ? -1 : 0)}>
                <Text style={[VoteStyle.vote, post.my_vote < 0 ? VoteStyle.voteDownEnabled:null]}>-</Text>
            </TouchableOpacity>
        </View>
    )
}